...
public class KahnsAlgorithmUsingBFSandIndegree {
  ...
  Approach:
  ---------
  - **Calculate indegree** of all nodes (number of incoming edges for each vertex).
  ...
  - If the result contains all V nodes, return it; otherwise, the graph has a cycle (not a DAG).
  ...
  - Space Complexity: O(V) for the queue, indegree array, and result.

  Related LeetCode Questions:
  ---------------------------
  - 207. Course Schedule (check if a topological ordering exists)
  - 210. Course Schedule II (return a valid topological order)
  - 269. Alien Dictionary (find order of characters)
  - 444. Sequence Reconstruction

  Follow-ups:
  -----------
  1. How to detect a **cycle** in a directed graph using Kahn’s algorithm?
  2. Can we use **DFS-based Topological Sort** instead of BFS? What are the trade-offs?
  3. How to modify Kahn’s Algorithm to produce **lexicographically smallest topological order**?
  4. How to handle graphs with **multiple valid topological orders**?*/
