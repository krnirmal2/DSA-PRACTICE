...
public class MinimumDaysToMakeMBouquets {
  ...
  denotes that the 'ith' rose will bloom on the 'arr[i]th' day.  You can only pick already bloomed roses that are adjacent
  to make a bouquet. You are also told that you require exactly 'k' adjacent bloomed roses to make a single bouquet.
  ...
  Result: -1
  Explanation: If we want to make 3 bouquets of 2 flowers each, we need at least 6 flowers. But we are given only 5 flowers,
  ...
  public static boolean possible(int[] arr, int day, int m, int k) {
    ...
    int cnt = 0;
    int noOfB = 0;
    // Count the number of bouquets: by traversing
    for (int i = 0; i < n; i++) {
      if (arr[i] <= day) {
        cnt++;
      } else {
        //          If we hit a flower not bloomed (arr[i] > day):
        noOfB += (cnt / k); // Convert cnt into bouquets: cnt / k and reset cnt = 0.
        cnt = 0;
      }
    }
    noOfB += (cnt / k); // At the end, also add leftover cnt / k.
    return noOfB >= m;
  }
