...
public class SumOfParticularSubmatrix {
  ...
  public static int[] solve(int[][] A, int[] B, int[] C, int[] D, int[] E) {
    ...
    // now take the uppper leftmost and  lower rightmost point to define the sub matrix need to
    ...
    if (B[1] != 0) {
      sum1 -= prefixSumMatrix[D[0]][B[1] - 1];
      result[0] = sum1;
    }

    if (B[0] != 0 && B[1] != 0) {
      sum1 += prefixSumMatrix[B[0] - 1][B[1] - 1];
      result[0] = sum1;
    }
    ...
    if (C[1] != 0) {
      sum2 -= prefixSumMatrix[E[0]][C[1] - 1];
      result[1] = sum2;
    }

    if (C[0] != 0 && C[1] != 0) {
      sum2 += prefixSumMatrix[C[0] - 1][C[1] - 1];
      result[1] = sum2;
    }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 2; i++) System.out.println("arr[" + i + "] = " + result[i]);
  }
}
