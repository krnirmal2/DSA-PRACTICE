...
public class ClimbingStairs {
  ...
  Pattern:
  - Dynamic Programming (DP) – Fibonacci relation:
      - ways(n) = ways(n-1) + ways(n-2)
      - Subproblems overlap → use memoization or bottom-up DP.
  ...
  Follow-ups:
  - What if steps allowed = {1, 2, k}?
  - What if some steps are broken (cannot be stepped on)?
  - Optimize to O(1) space using two variables.

  Time Complexity:
  - O(n) time, O(n) space with memoization. Can be optimized to O(1) space.
