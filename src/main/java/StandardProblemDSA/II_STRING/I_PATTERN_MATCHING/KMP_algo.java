...
public class KMP_algo {
   ...
   * <p>| Tip | Description | | ------------------------------- |
   * ------------------------------------------------------------------- | | 🔁 Use LPS | LPS avoids
   * unnecessary re-comparisons. | | 🧪 Dry Run | Always dry run the LPS computation with patterns
   * like `"ababaca"` | | 🔄 Reset only pattern index `j` | On mismatch, don’t restart from
   * beginning — jump to `lps[j-1]`. | | ⚡ Memory Efficient | Use `char[]` instead of `String` if
   * working with huge text streams. | | 🧩 Combine with Boyer-Moore | For hybrid fast search in
   * some engines. |
   *
   * <p>| Feature | KMP | Rabin-Karp | Z-Algorithm | | ------------------ | ---------------------- |
   * ------------------------------ | --------------------- | | Time Complexity | `O(N + M)` | `O(N
   * + M)` avg, `O(N*M)` worst | `O(N + M)` | | Use Case | Deterministic matching | Multiple
   * patterns | Prefix-based problems | | Preprocessing | LPS array | Hashing | Z-array | | Extra
   * Space | `O(M)` | `O(1)` (if single pattern) | `O(N + M)` | | Supports Wildcard? | With
   * modification | Yes (easier) | Harder to adapt |
   *
   ...
   * Classic KMP use case | | [686. Repeated String
   * Match](https://leetcode.com/problems/repeated-string-match/) | Find how many repeats needed to
   * contain pattern | | [796. Rotate String](https://leetcode.com/problems/rotate-string/) | Check
   * if one string is a rotation of another | | [459. Repeated Substring
   * Pattern](https://leetcode.com/problems/repeated-substring-pattern/) | Use KMP to detect cycles
   * | | [214. Shortest Palindrome](https://leetcode.com/problems/shortest-palindrome/) | KMP used
   * in reverse + concat approach | | [30. Substring with Concatenation of All
   * Words](https://leetcode.com/problems/substring-with-concatenation-of-all-words/) | Not direct
   * KMP but can optimize with string search |
   */
