...
public class DailyTemperature {
  public static int[] nextWarmDay(int[] temperatures) {

    // this is next greater element
    // take a stack where we will put the element index and then  compare
    // if the element at i is smaller then peek element then just add difference between their
    // indexex
    Stack<Integer> stackOfindex = new Stack<>();
    int n = temperatures.length;
    int[] result = new int[n];
    Arrays.fill(result, 0);
    //  we start from the last position and then we will compare
    for (int i = n - 1; i >= 0; i--) {
      //

      while (!stackOfindex.isEmpty() && temperatures[i] >= temperatures[stackOfindex.peek()]) {
        stackOfindex.pop();
      }
      if (!stackOfindex.isEmpty()) {
        result[i] = stackOfindex.peek() - i;
      }
      stackOfindex.push(i);
    }
    return result;
  ...
  public static void main(String[] args) {
    int[] dailyTemp = {73, 74, 75, 71, 69, 72, 76, 73};
    //        ArrayList<Integer> dailyTemp = new ArrayList<>(List.of(73, 74, 75, 71, 69, 72, 76,
    // 73));
    // can  be solved in brute force by two loop and arraylist
    int[] waitingList = nextWarmDay(dailyTemp);
