...
public class MinCostToConnectRopes {
  ...
  public static int minCost(int[] arr) {
    ...
    while (pq.size() > 1) {
      ...
      // as above two element by default give minimum element as MinHeap
      int firstElement = pq.poll();
      int secondElement = pq.poll();
      int sum = firstElement + secondElement;
      mincost += sum;
      // Step 3: after the two element we will add them back to the priority queue so we again add
      // to get min
      pq.add(sum);
    }
    return mincost;
