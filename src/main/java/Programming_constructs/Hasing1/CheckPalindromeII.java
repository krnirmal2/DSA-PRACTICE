...
public class CheckPalindromeII {
  ...
  //        // if odd then put in the hash and only one element having frequency one and other must
  // be in even no. or
  //        // all are same then only one element present in the hashmap and the frequency may be
  // anything (means size
  ...
  public static int solve(String A) {
    ...
    for (int i = 0; i < 26; i++) {
      if ((freq[i] % 2) != 0 && oneOdd) return 0;
      else if ((freq[i] % 2) != 0 && !oneOdd) oneOdd = true;
    }
    return 1;
  }
